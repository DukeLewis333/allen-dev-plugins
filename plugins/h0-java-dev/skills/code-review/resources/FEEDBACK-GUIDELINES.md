...
```text
建议：考虑将这个200行的方法拆分为多个职责单一的小方法，以提高可读性和可测试性。
...
```java
这里直接使用用户输入拼接 SQL 查询可能导致 SQL 注入攻击。
建议在 MyBatis XML 中使用 #{} 占位符来防止此类安全漏洞。

修改前（Mapper XML）：
SELECT * FROM bpm_order WHERE order_no = '${orderNo}'
...
```java
🟡 强烈建议：这个循环内的数据库查询会导致 N+1 问题。
影响：当数据量增大时，性能会显著下降
建议：先批量收集 ID，一次查出，再在内存中映射。
...
```java
🟢 建议：这个方法名 `doStuff` 不够清晰。
影响：降低代码可读性
建议：根据方法实际功能重命名，如 `processUserRegistration` 或 `validateEmailFormat`。
```
...
```java
🟢 建议：这个公共 API 方法缺少文档注释。
影响：其他开发者难以理解如何使用
建议：添加 Javadoc 注释说明参数、返回值和可能的异常。
...
- **静态代码分析**：SonarQube、SpotBugs、Checkstyle、PMD
